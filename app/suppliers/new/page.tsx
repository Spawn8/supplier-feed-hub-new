...
export default function NewSupplierWizardPage() {
  ...
  async function handleStep1Create() {
    ...
    try {
      if (sourceType === 'upload') {
        if (!file) { setS1Error('Please choose a file to upload.'); return }
    ...
    } catch (e: any) { setS1Error(e?.message || 'Upload failed'); return }
    ...
    startS1(async () => {
      ...
      if (!res.ok) {
        const j = await res.json().catch(()=>({})); setS1Error(j?.error || 'Failed to create supplier'); return
      }
    ...
    ;(async () => {
      ...
      try {
        ...
        if (!initializedRows.current) {
          const initial: Row[] =
            fieldArr.length > 0
              ? fieldArr.map(f => ({ field: f.key, source: existing.get(f.key) || '' }))
              : [{ field: '', source: '' }]
          setRows(initial)
  ...
  async function saveMappingsForSupplier() {
    ...
    if (!res.ok) {
      const j = await res.json().catch(()=>({})); throw new Error(j?.error || 'Failed to save mappings')
    }
  }
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const r = await fetch(`/api/suppliers/${supplierId}/details`).then(r=>r.json()).catch(()=>null)
  ...
  async function finishAndImport() {
    ...
    try {
      ...
      if (!lockedUidSourceKey) {
        const resUID = await fetch(`/api/suppliers/${supplierId}/set-uid-source`, {
          method: 'POST',
      ...
      // 4) Run import
      const resIngest = await fetch(`/api/suppliers/${supplierId}/ingest`, { method: 'POST' })
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {step === 1 && (
          <div className="rounded-2xl border bg-card p-6 space-y-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link href="/suppliers" className="btn">Cancel</Link>
            </div>
          </div>
        )}
        ...
        {step === 2 && supplierId && (
          <div className="rounded-2xl border bg-card p-6 space-y-4">
            ...
            {loadingMap ? (
              <div className="text-muted">Loading workspace fields & feed keys…</div>
            ) : (
              <>
                <p className="text-muted mb-2">
                  Left: <strong>workspace field</strong>. Right: <strong>feed key</strong>.
                </p>
                  ...
                  {rows.map((row, idx) => (
                    <div key={idx} className="grid grid-cols-1 md:grid-cols-10 items-center gap-2">
                      <div className="md:col-span-4">
                        <Select value={row.field} onChange={(e)=>setRow(idx, { field: e.target.value })}>
                      ...
                      <div className="md:col-span-5">
                        <Select value={row.source} onChange={(e)=>setRow(idx, { source: e.target.value })}>
                ...
                <div className="mt-4 flex items-center gap-2">
                  <Button
                    ...
                    onClick={() =>
                      startMap(async () => {
                        try { await saveMappingsForSupplier(); setStep(3) }
                        catch (e:any) { setMapError(e?.message || 'Failed to save mappings') }
                      })
                    }
                  >
        ...
        {step === 3 && supplierId && (
          <div className="rounded-2xl border bg-card p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button onClick={() => setStep(2)} disabled={isFinishing}>Back</Button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="md:col-span-2">
                ...
                {!lockedUidSourceKey ? (
                  <>
                    <Select value={uidSourceKey} onChange={(e)=>setUidSourceKey(e.target.value)}>
                      <option value="">— Select feed key to use as UID —</option>
                      {feedKeys.map(k => <option key={k} value={k}>{k}</option>)}
                    </Select>
                    <p className="text-xs text-muted mt-1">
                      This must uniquely identify a product within this supplier’s feed (e.g. code, SKU, EAN). Once saved, it cannot be changed.
                    </p>
                  </>
                ) : (
                  <div className="rounded border px-3 py-2 bg-gray-50">
                    <div className="text-sm"><strong>UID source (locked):</strong> {lockedUidSourceKey}</div>
            ...
            <div className="flex items-center gap-2">
              <Button
                variant="primary"
                disabled={isFinishing || !name.trim() || (!lockedUidSourceKey && !uidSourceKey)}
                onClick={finishAndImport}
              >
                {isFinishing ? 'Finishing… Importing…' : 'Finish & Import'}
              </Button>
              <Link href="/suppliers" className="btn" aria-disabled={isFinishing}>Cancel</Link>
            </div>
            ...
            {finishResult && (
              <div className="mt-4 rounded-2xl border bg-white p-4">
                ...
                <div className="text-sm text-muted">
                  Type: {finishResult.type || 'unknown'} • Ingestion ID: {finishResult.ingestion_id}
                </div>
                <div className="mt-2 grid grid-cols-3 gap-2 text-center">
                  <div className="rounded bg-gray-100 p-3">
                    <div className="text-xs text-muted">Total</div>
                    <div className="text-lg font-semibold">{finishResult.stats?.total ?? 0}</div>
                  </div>
                  <div className="rounded bg-gray-100 p-3">
                    <div className="text-xs text-muted">OK</div>
                    <div className="text-lg font-semibold">{finishResult.stats?.ok ?? 0}</div>
                  </div>
                  <div className="rounded bg-gray-100 p-3">
                    <div className="text-xs text-muted">Errors</div>
                    <div className="text-lg font-semibold">{finishResult.stats?.errors ?? 0}</div>
                  </div>
                </div>

                <div className="mt-4 flex gap-2">
                  <Link href={`/suppliers/${supplierId}/mapped`} className="btn">View mapped items</Link>
                  <Link href={`/suppliers/${supplierId}/raw`} className="btn">View raw items</Link>
                  <Link href="/suppliers" className="btn btn-primary">Done</Link>
                </div>
              </div>
            )}
          </div>
